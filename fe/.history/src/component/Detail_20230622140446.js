...
export default function Detail() {
  ...
  if (isLoading) {
    ...
    return (
      <>
        ...
        <Helmet>
        <title>{`${product.tensp} - Gaming Zone`}</title>
        <meta property="og:title" content={`${product.tensp} - Gaming Zone`} key="title" />
        <meta property="og:description" content={product.tensp} />
        <meta name="description" content={product.tensp} />
        <meta property="og:image" content={product.imgUrl} />
        <meta property="og:image:url" content={product.imgUrl} />
        <meta property="og:image:secure_url" content={product.imgUrl} />
  ...
  <div className="mota-chinh-content" style={{ maxHeight: showMore ? "none" : "1000px" }}>
    <div dangerouslySetInnerHTML={{ __html: product.mota_chinh }} />
    
  </div>
  {!showMore && <div className="readmore" onClick={() => setShowMore(true)}>Xem thêm</div>}
</div>
