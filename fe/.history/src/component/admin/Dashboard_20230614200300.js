...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    }, 1000); // Gửi yêu cầu liên tục sau mỗi giây
  ...
  useEffect(() => {
    const handleActiveUsers = (response) => {
      const activeUsers = response.result.totalsForAllResults['rt:activeUsers'];
      setTotalVisitors(activeUsers);
    };

    ReactGA.api({
      path: '/realtime/total',
      callback: handleActiveUsers,
    });
  }, []);
  useEffect(() => {
  ...
  return (
    <>
      ...
      <Typography.Text type="secondary">
        Lượt xem hôm nay: {totalVisitors}
      </Typography.Text>
    </Card>
        </Col>
        <Col span={8}>
