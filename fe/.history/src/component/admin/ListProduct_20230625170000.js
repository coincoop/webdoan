...
const ListProduct = () => {
  ...
  const getProducts = async () => {
    ...
    await Promise.all(
      prod.map(async (product) => {
        ...
        // Khởi tạo trạng thái visible cho từng sản phẩm
  ...
  const updateStatusProd = async (productId) => {
    try {
      ...
      await axios.put(`${API_URL}admin/adproducts/tt/${productId}`, {
        visible: newStatus,
      });
...
<Button
  ...
  className="ant-btn"
  size="large"
  icon={product.visible === 0 ? <EyeOutlined /> : <EyeInvisibleOutlined />}
