...
const Dashboard = () => {
  ...
  useEffect(() => {
    // Khởi tạo Google Analytics với mã theo dõi của bạn
    ReactGA.initialize('G-5W93GCZSFP');
    ReactGA.pageview(window.location.pathname);

    const updateVisitors = () => {
      setTotalVisitors(ReactGA.getting('activeUsers'));
    };


    // Cập nhật số người xem mỗi giây
    const interval = setInterval(updateVisitors, 1000);
